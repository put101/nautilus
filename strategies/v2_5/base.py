...
class PUT101Strategy(Strategy):
    ...
    def ingress_tracker(self, instrument_id:typing.Optional[str], name: str, value: float, ts:int):
        sender = self.questdb
        try:
            sender.row(
                'indicators',
                symbols={
                    'instrument_id': self.instrument_id.value if instrument_id is None else instrument_id,
                    'id': self.conf.IDENTIFIER,
                    'indicator': name
                },
                columns={
                    'value': value,
    ...
    def ingress_event(self, event: Event):
        ...
        try:
            sender.row(
                'events',
                symbols={
                    'symbol': self.instrument_id.value,
